...
def melSpectra(y, sr, wsize):  #sound:(132300,) , sr=48000, wsize=0.04, hsize =0.04
    cnst = 1 + (int(sr * wsize) / 2) #883#The number of rows in the STFT matrix
    y_stft_abs = np.abs(librosa.stft(y, #132300
                                     win_length=int(sr * wsize), #1764
                                     hop_length=int(sr * wsize), #1764
                                     center=False,
...
class PredictThread2(threading.Thread):
    def __init__(self, recorder: RecordThread2):
        ...
        #self.model = load_model('/Users/liukuangxiangzi/PycharmProjects/PhonemeNet/model/result_model/logmel_cnn_ep50_bs32_nocenter.h5')
        #self.model_audio2viseme = load_model('/Users/liukuangxiangzi/PycharmProjects/PhonemeNet/model/result_model/logmel_cnn_ep50_bs32_nocenter.h5')
        #self.model_viseme2animation  = load_model('/Users/liukuangxiangzi/PycharmProjects/PhonemeNet/viseme2animation_ep300.h5')
        self.model_audio2viseme = load_model('model/logmel_cnn_ep50_bs32_nocenter.h5')
        self.model_viseme2animation = load_model('model/model_w2v2AniPara/model_3132/p2a_cnn_ep400_31_32_1188_appendAni.h5')  #p2a_cnn_ep300_5648.h5 #p2a_cnn_ep400_31_32_1188_appendAni  #p2a_cnn_ep400_31_32_1188.h5
        self.wsize=0.04
        self.hsize =0.04
        self.ctxWin = 5
        self.count = 1
    ...
    def run(self):
        ...
        time_step = np.zeros((6, 128))#, dtype='f16')
        ...
        while not finished_processing:
            ...
            if data is not None:
                ...
                if len(self.melFrames_3) == 3:
                    if count == 1:
                        count += 1
                        #print('n=1', np.diff(melFrames, n=1, axis=0).shape) #(2, 64)
                        #print(np.squeeze(np.diff(melFrames_3, n=1, axis=0)).shape)
                ...
                if len(self.time_step_features) == 6:
                    ...
                    ##print(to_categorical(np.argmax(y), num_classes=13))
