...
from keras.layers import Dense, Flatten, Conv1D, LeakyReLU, Softmax
...
#load data
x = np.load('/Users/liukuangxiangzi/PycharmProjects/PhonemeNet/data_p2a/w2v_1195_31_32_softmax.npy') #
...
if not resume_training:
    ...
    l1 = Conv1D(32, 3, strides=2,
                padding='same',
                kernel_initializer=initializer,
                name='phoneme_Conv1D_1')(net_in)
    l1af = LeakyReLU(0.02)(l1)


    l2 = Conv1D(32, 3,strides=2,
                padding='same',
                kernel_initializer=initializer,
                name='phoneme_Conv1D_2')(l1af)
    l2af = LeakyReLU(0.02)(l2)

    l3 = Conv1D(64, 3,strides=2,
                padding='same',
                kernel_initializer=initializer,
                name='phoneme_Conv1D_3')(l2af)
    l3af = LeakyReLU(0.02)(l3)

    l4 = Conv1D(64, 3,strides=2,
                padding='same',
                kernel_initializer=initializer,
                name='phoneme_Conv1D_4')(l3af)
    l4af = LeakyReLU(0.02)(l4)

    d1 = Dense(128, kernel_initializer=initializer, name='d1')(l2af)
    d1af = LeakyReLU(0.02)(d1)

    d2 = Dense(64, kernel_initializer=initializer, name='d2')(d1af)
    d2af = LeakyReLU(0.02)(d2)
    d3 = Dense(32, kernel_initializer=initializer, name='d3', activation='tanh')(d2af)
    ...
    att_l5 = Conv1D(1, 3,strides=1,
                    padding='same',
                    kernel_initializer=initializer,
                    name='att_Conv1D_5')(att_l4af)
    att_l5af = LeakyReLU(0.02)(att_l5)

    att_l6 = Dense(1, kernel_initializer=initializer, name='out_att')(att_l5af)
    att_out = Softmax()(att_l6)


    bmm = K.batch_dot(K.permute_dimensions(d3,(0,2,1)), att_out)
...
plot_model(model, to_file='model_p2a_cnn_att_8_5att_plot.png', show_shapes=True, show_layer_names=True)
#
#
model.save('selfattention_softmax_VP' +'.h5')
